function selector(e) {
  return document.querySelector(e);
}
let personID = document.querySelector('.personID');

let username = sessionStorage.getItem("userid");
if (username == ' ' || username == null) {
  personID.innerHTML = '未登录';
  console.log('你好啊');
} else {
  personID.innerHTML = username;
  console.log('我不好');
}
let say1 = document.querySelector(".say.s1>span");
let say2 = document.querySelector(".say.s2>span")
let index1 = 0;
let index2 = 0;

function randomText() {
  let texts1 = [
    "嗯嗯！！你一定可以的！我们一起加油哇！！",
    "俺刚刚也被骗了！希望创始主能够拯救我们呜呜",
    "对！！我刚刚才出来的，俺可是认真学习的好鸭鸭",
    "那根是俺的呀！不可以偷俺的东西！！快来救我！!",
    "(｡>∀<｡)我也是一样！！！"
  ];
  let texts2 = [
    `✧٩(ˊωˋ*)و✧总有一天俺也会设计出自己的剧本滴！`,
    "( ´•̥̥̥ω•̥̥̥` )咦！刚刚是哪个剧本骗了我呀呜呜呜呜",
    "(*ﾟﾛﾟ)!!百科里面的诈骗方案好惊险，俺学会了好多东西",
    "哈哈哈哈~棒棒糖，想偷偷藏两个在俺口袋里面...Ｏ(≧▽≦)Ｏ",
    "(｡>∀<｡)醒来我一定要去社区看看大家发了什么有趣的知识点！！！"
  ];

  index1++;
  index2++;

  // console.log(index1);
  // index1 = Math.floor(Math.random() * texts1.length);
  if (index1 > texts1.length - 1) {
    index1 = 0;
  }
  if (index2 > texts2.length - 1) {
    index2 = 0;
  }
  say1.style.opacity = 0; // 设置透明度为0，进行过渡效果
  setTimeout(function () {
    say1.innerHTML = texts1[index1];
    say1.style.opacity = 1; // 设置透明度为1，过渡显示出文本
  }, 1000); // 300毫秒后更改文本内容并显示
  say2.style.opacity = 0; // 设置透明度为0，进行过渡效果
  setTimeout(function () {
    say2.innerHTML = texts2[index2];
    say2.style.opacity = 1; // 设置透明度为1，过渡显示出文本
  }, 1000); // 300毫秒后更改文本内容并显示
};

setInterval(randomText, 5000);


let navPresonA = document.querySelector('.navPreson>a');
// navPresonA.preventDefault();
navPresonA.addEventListener('click', function (e) {
  e.preventDefault();
  if (personID.textContent == '未登录') {
    window.location.href = "login1.html";
  } else {
    window.location.href = "center.html";
    // console.log(window.location.href);
  }
})

// 改变头像
let Pic=document.querySelector('.personHead>img');
const p2 = new Promise((resolve, reject) => {
  const xhr = new XMLHttpRequest();
  xhr.open('GET', 'http://47.113.231.211:3000/users');
  xhr.setRequestHeader('token', `${sessionStorage.getItem("sign1")}`);
  // xhr.setRequestHeader('token', 'eyJhbGciOiJIUzI1NiJ9.eyJqdGkiOiJhMzJkYTM2ZTYxY2I0MjJiOTc3NmY5ODJmNTk5Njg4ZCIsInN1YiI6IjAiLCJpc3MiOiJzZyIsImlhdCI6MTY5NzYzODg1OH0.5-zD7hDvC-iCWyqMyNMlmdF8XTkBx8HvuQ8NtyUD5F8');
  xhr.send();
  xhr.onreadystatechange = function () {
    if (xhr.readyState === 4) {
      if (xhr.status >= 200 && xhr.status < 300) {
        // 成功
        resolve(xhr.response);
      } else {
        // 失败
        reject(xhr.status);
      }
    }
  }
});

// 指定回调
p2.then(function (value) {
  value = JSON.parse(value);
  let { code, data,msg} = value;
  console.log("你到底要干嘛啊",value);
  if( data.userValue.pic==null ||data.userValue==null){
    Pic2.src ='data:image/png;base64,';
    sessionStorage.setItem("pic", 'data:image/png;base64,');
  }else{
    Pic.src = 'data:image/png;base64,' + data.userValue.pic;
    sessionStorage.setItem("pic", `${data.userValue.pic}`);
  }

}, function (reason) {
  console.log("失败了啊啊啊");
  console.error(reason);
})

const emptyAd =document.querySelector('.emptyA.d');
const Xd =document.querySelector('.emptyA.d .x');
const qd =document.querySelector('.emptyA.d .q');

// JavaScript 代码片段
var links = document.querySelectorAll('a.nax-tag');
links.forEach(function(link) {
  link.addEventListener('click', function(event) {
    var b = document.querySelector('.personID');
    if (b.textContent === '未登录') {
      event.preventDefault();
      emptyAd.style.display='block';
    }
  });
});

Xd.addEventListener('click',function(){
  emptyAd.style.display='none';
})

qd.addEventListener('click',function(){
  window.location.href = "login1.html";
})