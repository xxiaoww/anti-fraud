...
// 这是第几个剧本的意思
liIndex = sessionStorage.getItem("scriptId");
...
// 接收结局
// const url1 = 'http://47.113.231.211:3000/script/play/end';
// const headers1 = {
//     'Content-Type': 'application/json'
// };

// const payload1 = {
//     scriptId: parseInt(liIndex),
//     influence1: 50,
//     influence2: 50,
//     influence3: 50,
//     influence4: 0
...
//     .then(data1 => {
//         console.log("你猜猜这个是什么", data1);
//         // console.log(data1.data);
//         // normalEnd = data1.data.normalEnd;
//         // console.log(normalEnd);
//         // specialEnd = data1.data.specialEnd;
//     })
//     .catch(error1 => {
...
fetch(url, {
    ...
    .then(data => {
        console.log(data);
        let { scriptInfluenceName, scriptMsg, scriptNodes } = data.data;
        ...
        for (let i = 0; i < scriptNodes.length; i++) {
            ...
            // console.log("你猜猜这是什么",scriptNodes[i].nodeId);
        ...
        window.addEventListener('mousemove', function (event) {
            ...
            if (mouseX >= card1Rect.left && mouseX <= card1Rect.right) {
                ...
                if (mouseX < card1Rect.left) {
                    ...
                    if (scriptNodes[cardId].leftChoice != null) {
        ...
        window.addEventListener('click', function (event) {
            ...
            if (mouseX <= card1Rect.left) {
                ...
                if (scriptNodes[cardId].leftChoice != null) {
                    influence1 += scriptNodes[cardId].rightChoice.influence1;

                    i1.style.setProperty('--gradient-start', `${influence1}`);
                    i1.style.setProperty('--gradient-stop', `${100 - influence1}`);

                    influence2 += scriptNodes[cardId].rightChoice.influence2;
                    i2.style.setProperty('--gradient-start', `${influence2}`);
                    i2.style.setProperty('--gradient-stop', `${100 - influence2}`);

                    influence3 += scriptNodes[cardId].rightChoice.influence3;
                    i3.style.setProperty('--gradient-start', `${influence3}`);
                    i3.style.setProperty('--gradient-stop', `${100 - influence3}`);

                    influence4 += scriptNodes[cardId].rightChoice.influence4;
                    i4.style.setProperty('--gradient-start', `${influence4}`);
                    i4.style.setProperty('--gradient-stop', `${100 - influence4}`);
                    simulate(temp);
                    // if(!t) return; 
...
back.addEventListener('click', function (e) {
    e.stopPropagation();
    history.back();
    // window.location.href = "mStart.html"; // 跳转到指定链接
})
...
const ss = document.querySelector('.end>.ss');
const sX = document.querySelector('.end>.ss .closed');
const sGood = document.querySelector('.end>.ss .good');
...
sGood.addEventListener('click', function () {
    ...
    // 获取选中的单选按钮的值
    let selectedValue = choiceContainer.querySelector('input[name="option"]:checked').value;
    console.log("selectedValue的值为：",selectedValue);
    if (selectedValue) {
        const url = 'http://47.113.231.211:3000/script/score';
        const headers = {
            'Content-Type': 'application/json',
            'token': `${sessionStorage.getItem("sign1")}`
        };
